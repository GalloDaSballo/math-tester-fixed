...
contract TwTAP is TWAML, ERC721, ReentrancyGuard, Ownable {
    ...
    function claimable(uint256 _tokenId) public view returns (uint256[] memory) {
        ...
        unchecked {
            ...
            votes = uint256(position.tapAmount) * uint256(position.multiplier); /// @audit Why re-compute?
        } /// @audit can position be altered?
        ...
        if (position.lastActive < week) {
            week = position.lastActive; /// @audit "Catching up"
        }
        ...
        for (uint256 i; i < len;) {
            ...
            result[i] = ((votes * net) / DIST_PRECISION) - claimed[_tokenId][i]; /// @audit Check this for claimed actually offsetting correctl
            unchecked {
                ++i;
            }
        }
        ...
        returns (uint256 tokenId)
    {
        ...
        if (magnitude >= pool.cumulative * 4) revert NotValid(); // You can lock 1/4 of Max and still get max
        uint256 multiplier = computeTarget(dMIN, dMAX, magnitude, pool.cumulative); /// 0, 1 MLN -> Magnitude, Cumulative

        // Calculate twAML voting weight
        bool divergenceForce; /// @audit You lose voting power with small amount, but you get better multiplier?
        ...
        tokenId = ++mintedTWTap;
        uint256 votes = _amount * multiplier; /// @audit Can we get this to blow up?
    ...
    {
        {
            ...
            if (_to != owner_) {
                _requireClaimPermission(_to, _tokenId); /// @audit We can claim to Approved AMM or w/e so we can cause total loss
            } /// Or to L0
        }
    ...
    function advanceWeek(uint256 _limit) public nonReentrant {
        ...
        uint256 week = lastProcessedWeek; // 0
        uint256 goal = currentWeek(); // 1
        unchecked {
            if (goal - week > _limit) { /// @audit Overflows?
                goal = week + _limit;
            }
        }
        uint256 len = rewardTokens.length;
        while (week < goal) { // 0
            ...
            WeekTotals storage next = weekTotals[++week]; /// @audit pre-increment to next // 1
    ...
    function setVirtualTotalAmount(uint256 _virtualTotalAmount) external onlyOwner {
        VIRTUAL_TOTAL_AMOUNT = _virtualTotalAmount; /// @audit No min / max
    }
    ...
    function setMinWeightFactor(uint256 _minWeightFactor) external onlyOwner {
        MIN_WEIGHT_FACTOR = _minWeightFactor; /// @audit No min / max
    }
    ...
    function setMaxRewardTokensLength(uint256 _length) external onlyOwner {
        ...
        maxRewardTokens = _length; /// @audit No min / max and cannot delete the list
    }
        ...
        unchecked {
            for (uint256 i; i < len; ++i) {
                ...
                if (amount > 0) {
                    ...
                    claimed[_tokenId][i] += amount; /// @audit , if we make this overflow, we get free stuff
